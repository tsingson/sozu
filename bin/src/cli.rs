...
pub enum SubCmd {
  ...
  #[structopt(name = "main", about = "start a new main process (internal command, should not be used directly)")]
  Main {
    #[structopt(long = "fd", help = "IPC file descriptor")]
    fd: i32,
    #[structopt(long = "upgrade-fd", help = "upgrade data file descriptor")]
    upgrade_fd: i32,
    #[structopt(long = "command-buffer-size", help = "Main process channel buffer size",  default_value = "1_000_000")]
    command_buffer_size: usize,
    #[structopt(long = "max-command-buffer-size", help = "Main process channel max buffer size")]
    max_command_buffer_size: Option<usize>,
  },
