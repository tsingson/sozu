...
use nom::Offset;
use nom::{bytes::streaming::take_while, combinator::recognize, sequence::tuple, IResult};
...
impl Slice {
    ...
    pub fn consume(self, sz: usize) -> (usize, Option<Slice>) {
        if sz >= self.len as usize {
            ...
            (0,  Some(Slice { start: start + (sz as u32), len: len - (sz as u16), meta }))
        }
    }
}
...
impl CopyingSlice {
    pub fn consume(self, sz: usize) -> (usize, Option<CopyingSlice>) {
        match self {
            ...
            CopyingSlice::Static(s) => if sz >= s.len() {
                (sz - s.len(), None)
            } else {
