...
use nom::Offset;
use super::{compare_no_case, is_header_value_char};
use super::{token, sp, vchar_1, crlf, tag, one_of};
use nom::{
    IResult,
    sequence::tuple,
...
impl Slice {
    ...
    pub fn new(buffer: &[u8], data: &[u8], meta: Meta) -> Slice {
        ...
        assert!(offset <= u32::MAX as usize, "header slices should not start at more than 4GB from its beginning");
        assert!(data.len() <= u16::MAX as usize, "header slices should not be larger than 65536 bytes");
    ...
    pub fn modify(&mut self, buffer: &mut crate::pool::Checkout, new_value: &[u8]) -> bool {
        ...
        if new_value.len() <= self.len as usize {
            ...
            */
            (buffer.data_mut()[self.start as usize..self.start as usize+new_value.len()]).copy_from_slice(new_value);
...
impl Header {
    ...
    pub fn name<'a>(&self, buffer: &'a [u8]) -> Option<&'a str> {
        self.name.data(buffer).map(|s| &s[..s.len()])
            .and_then(|s| std::str::from_utf8(s).ok())
    }

    pub fn value<'a>(&self, buffer: &'a [u8]) -> Option<&'a str> {
        self.value.data(buffer).map(|s| &s[..s.len()])
            .and_then(|s| std::str::from_utf8(s).ok())
    }
...
impl RequestLine {
    ...
    pub fn uri<'a>(&self, buffer: &'a [u8]) -> Option<&'a str> {
        self.uri.data(buffer) //.map(|s| &s[..s.len() - 1])
            .and_then(|s| std::str::from_utf8(s).ok())
    }
    ...
    pub fn to_rrequest_line(&self, buffer: &[u8]) -> Option<super::RRequestLine> {
        let method = match self.method.meta {
            ...
            Meta::Method(Method::Custom) => match self.method.data(buffer) {
                None => return None,
                Some(s) => super::Method::Custom(String::from(unsafe { std::str::from_utf8_unchecked(s)}))
            },
            _ => return None,
        };
        ...
        Some(super::RRequestLine { method, uri, version })
    }
}
...
mod tests {
    ...
    fn parse_modify() {
        ...
        assert_eq!(std::str::from_utf8(buffer.data()).unwrap(),
    ...
    fn parse_post_request() {
        let mut pool = crate::pool::Pool::with_capacity(1, 1, 16384);
        let mut buffer = pool.checkout().unwrap();
        buffer.write(b"POST /index.html HTTP/1.1\r\n\
          Host: localhost:8888\r\n\
        ...
        for header in request.1.iter_mut() {
            println!("\t{:?} ({:?}) -> {:?}", header.name(buffer.data()), header.name.meta, header.value(buffer.data()));
